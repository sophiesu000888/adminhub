...
const _MOBILE_MAX_CHUNKS = 5; // 手機最多讀 5 個 chunk（約 1500 筆）
...
function startChunkedSync(moduleName, currentUser, onRemoteUpdate) {
  ...
  // 手機：用 get() 一次性讀取，不用 onSnapshot 避免反覆觸發渲染
  if (_isMobile) {
    (async () => {
      try {
        setSyncStatus('⟳ 載入中…', 'sync');
        const metaSnap = await db.collection('modules').doc(`${moduleName}_meta`).get();
        if (!metaSnap.exists) { onRemoteUpdate([]); setSyncStatus('✓ 已儲存', 'saved'); return; }
        const meta = metaSnap.data();
        if (meta.lastSaved) setLastSaved(meta.lastSaved.displayName, meta.lastSaved.uid === currentUser.uid, meta.lastSaved.timestamp);
        const chunkCount = meta.chunkCount || 0;
        const startChunk = Math.max(0, chunkCount - _MOBILE_MAX_CHUNKS);
        const fetches = [];
        ...
        const combined = [];
        snaps.forEach(snap => {
          if (snap.exists) snap.data().rows.forEach(r => combined.push(restoreFromFirestore(r)));
        });
        onRemoteUpdate(combined);
        setSyncStatus('✓ 已儲存', 'saved');
      } catch(e) {
        console.error('mobile chunkedSync error', e);
        setSyncStatus('✗ 載入失敗', 'err');
      }
    })();
    return () => {};
  }
  ...
  return db.collection('modules').doc(`${moduleName}_meta`).onSnapshot(async (metaSnap) => {
    ...
    if (!metaSnap.exists) {
      ...
      if (oldSnap.exists) {
        ...
        if (data.lastSaved) setLastSaved(data.lastSaved.displayName, data.lastSaved.uid === currentUser.uid, data.lastSaved.timestamp);
        const content = data.content ? restoreFromFirestore(data.content) : null;
        onRemoteUpdate(content, true);
      } else {
        onRemoteUpdate([]);
      }
      setSyncStatus('✓ 已儲存', 'saved');
    ...
    if (_savingModules.has(moduleName)) { setSyncStatus('✓ 已儲存', 'saved'); return; }
    if (metaSnap.metadata && metaSnap.metadata.hasPendingWrites) { setSyncStatus('✓ 已儲存', 'saved'); return; }
